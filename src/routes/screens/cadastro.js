...
const Cadastro = ({navigation}) => {
  ...
  const buscarCep = async cep => {
    ...
    try {
      ...
      const {street, neighborhood, city, state, location} = response.data;
      ...
      if (location && location.coordinates) {
        ...
        setLatitude(latitude); // Novo estado
        setLongitude(longitude); // Novo estado
      }
    } catch (error) {
        ...
        <ScrollView
          ...
          <SafeAreaView style={styles.cadastroArea}>
            ...
            <TouchableOpacity style={styles.gnrbtn} onPress={toggleList}>
              ...
              {selectedSexuality && (
                <Text style={styles.gnrtxt}>{selectedSexuality}</Text>
              )}
            </TouchableOpacity>
            ...
            <TextInput
              ...
              value={cep}
              onChangeText={text => {
                setCep(text); // Atualiza o estado do CEP
                if (text.length === 8) {
                  // Verifica se o CEP tem 8 caracteres
                  buscarCep(text); // Chama a função automaticamente
                }
              }}
              keyboardType="numeric"
              maxLength={8}
